...
class CarWashStation:
    ...
    def calculate_washing_price(self, car: Car) -> float:
        pollution_diff = abs(car.clean_mark - self.clean_power)
        rating_factor = self.average_rating / self.distance_from_city_center
        return round(car.comfort_class * pollution_diff * rating_factor, 1)

    def wash_single_car(self, car: Car) -> None:
        car.clean_mark = self.clean_power

    def rate_service(self, rate: int) -> None:
        ratings_sum = self.count_of_ratings * self.average_rating + rate
